...
class LayoutCubit extends Cubit<LayoutStates> {
  ...
  void createMyCommunity({required String communityName, required String communityDescription}){
    String topicName = "${communityName}${userID!}";
    ...
    FirebaseStorage.instance
      ...
      value.ref.getDownloadURL().then((imageUrl) async {
        ...
        if( RegExp(r'^[a-zA-Z0-9-_.~%]{1,900}$').hasMatch(topicName) == true )
        {
          ...
          await FirebaseMessaging.instance.subscribeToTopic(topicName);      // Todo: to get notifications when anyone else add a new post on this community
        }
        await getMyAllCommunitiesData();
        emit(CreateMyCommunitySuccessfullyState());
      });
    }).catchError((onError) {
      emit(FailedToCreateMyCommunityState());
    });
  }
  ...
  Future<void> deleteCommunity({required String communityID, required String communityName}) async {
    String topicName = "${communityName}${userData!.userID ?? userID}";
    emit(DeleteCommunityLoadingState());
    try {
      await FirebaseFirestore.instance
          .collection('users')
          .doc(userID ?? userData!.userID)
          .collection('own_communities')
          .doc(communityID)
          .delete();
      // Todo: made this condition to use on validate name for Topic as + not validate like C++ this isn't a validate name for Topic
      if( RegExp(r'^[a-zA-Z0-9-_.~%]{1,900}$').hasMatch(topicName) == true )
  ...
  void addToJoinedCommunity(
    ...
    sendNotificationAfterJoinCommunity(
        receiverFirebaseMessagingToken:
            communityModel.authorFirebaseMessagingToken!,
    ...
    await FirebaseFirestore.instance
        ...
        .catchError((error) {
      emit(FailedToAddToJoinedCommunityState());
    });
    await FirebaseMessaging.instance.subscribeToTopic("${communityModel.communityName!.substring(0,3)}${communityModel.authorID!.substring(0,3)}"); // Todo: use it to get notifications after anyone create post on this community
    await getMyAllCommunitiesData();
  ...
  Future<void> leaveCommunity({required String communityID, required String communityName, required String communityAuthorID}) async {
    String topicName = "${communityName}${communityAuthorID}";
  ...
  void updateCommunity(
      {required CommunityModel model, required String communityID}) async {
        ...
        if (userDoc.id != userID) {
          userDoc.reference.collection('own_communities').get().then((items) {
            for (var communityItem in items.docs) {
              if (joinedCommunitiesID.contains(communityItem
                  .id)) // Todo: this mean that this community on my joined_communities
              {
                myCommunitiesData
                    .add(CommunityModel.fromJson(json: communityItem.data()));
  ...
  void addComment(
    ...
    try {
      final commentModel = CommentModel(comment, userID ?? userData!.userID,
          userData!.image, userData!.userName, timeNow, postID);
      await FirebaseFirestore.instance
          .collection('users')
          .doc(postModel.communityAuthorID)
          .collection('own_communities')
          .doc(postModel.communityID)
          .collection('posts')
          .doc(postID)
          .collection('comments')
          .add(commentModel.toJson());
      await getAllComments(
          communityAuthorID: postModel.communityAuthorID!,
  ...
  Future<void> getAllComments(
      {required String communityID,
      required String communityAuthorID,
      required String postID}) async {
    comments.clear();
    commentsID.clear();
              ...
              for (var communityDoc in communityDocs.docs) {
                if (communityDoc
                  ...
                  otherCommunitiesData
                      .add(CommunityModel.fromJson(json: communityDoc.data()));
  ...
  Future<void> createPostWithoutImage(
    ...
    if (userID != communityAuthorID)
      sendNotifyAfterAddPost(
          communityID, communityAuthorID, communityName, communityImage);
    await getMyAllPostsData();
  ...
  Future<void> getJoinedCommunitiesID() async {
    await FirebaseFirestore.instance
      ...
      debugPrint(
          "Joined communities ID is ................ $joinedCommunitiesID");
    });
  }
  ...
  Map<String, bool> likesStatus =
      {}; // Todo: store postID as a key and status as a value
  // Todo: This contain own communities with communities that I have joined to display on Home Screen
                    ...
                    if (value.docs.isNotEmpty) {
                      for (var element in value.docs) {
                        if (element.id == userID)
                          likesStatus.addAll(
                              {item.id: true}); // Todo: item.id == postID
                        postsData.add(PostModel.fromJson(json: item.data()));
                        postsID.add(item.id);
                      }
                    } else {
    ...
    body: {
      {
        ...
        "notification": {
          "title": "New Post",
          "body":
              "${userData!.userName} add a new Post on %$communityName Community",
